...
class JaniceMarketAPI {
    ...
    func createAppraisal(items: [String: Int]) async throws -> Data {
        ...
        let query = "SELECT type_id, en_name FROM types WHERE type_id IN (\(itemIds.joined(separator: ",")))"
        ...
        if case .success(let rows) = result {
            for row in rows {
                let typeIdInt = (row["type_id"] as? Int64).map(Int.init) ?? (row["type_id"] as? Int) ?? 0
        ...
        requestData.append("Content-Disposition: form-data; name=\"~request~\"\r\n\r\n".data(using: .utf8)!)
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            throw NSError(domain: "JaniceMarketAPI", code: -1, userInfo: [NSLocalizedDescriptionKey: "HTTP请求失败"])
        }
