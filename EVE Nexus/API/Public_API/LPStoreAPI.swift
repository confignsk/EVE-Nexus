...
class LPStoreAPI {
    ...
    // MARK: - 公共方法
    func fetchLPStoreOffers(corporationId: Int, forceRefresh: Bool = false) async throws -> [LPStoreOffer] {
        // 如果不是强制刷新，尝试从数据库获取
        if !forceRefresh {
    ...
    private func loadFromDatabase(corporationId: Int) throws -> [LPStoreOffer]? {
        ...
        if case .success(let rows) = CharacterDatabaseManager.shared.executeQuery(query, parameters: [corporationId]),
           let row = rows.first,
    ...
    private func saveLPStoreOffers(corporationId: Int, offersData: Data) {
        ...
        _ = CharacterDatabaseManager.shared.executeQuery(query, parameters: [
            corporationId,
            offersString
        ])
