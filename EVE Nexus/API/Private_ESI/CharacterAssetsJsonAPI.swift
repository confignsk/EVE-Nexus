...
public class CharacterAssetsJsonAPI {
    ...
    // MARK: - Cache Methods
    private func getCacheFilePath(characterId: Int, timestamp: Date? = nil) -> URL? {
    ...
    private func cleanOldCacheFiles(characterId: Int) {
    ...
    private func fetchAllAssets(
        ...
        let baseUrlString = "https://esi.evetech.net/latest/characters/\(characterId)/assets/?datasource=tranquility"
    ...
    // 获取多语言星系信息
    private func fetchSystemInfo(solarSystemId: Int, databaseManager: DatabaseManager) async -> SysInfo? {
        // 构建查询语句获取星系信息和区域信息
        let query = """
            SELECT region_id, system_security from universe where solarsystem_id = ?
        """
        
        if case .success(let rows) = databaseManager.executeQuery(query, parameters: [solarSystemId]) {
    ...
    private func fetchLocationInfo(
        ...
        characterId: Int,
        databaseManager: DatabaseManager
    ) async throws -> (name: String?, typeId: Int?, systemId: Int?, securityStatus: Double?, regionId: Int?) {
        ...
        if locationType == "solar_system" {
            ...
            if let systemInfo = await fetchSystemInfo(solarSystemId: Int(locationId), databaseManager: databaseManager) {
        ...
        if locationType == "station" {
            if let stationInfo = try? await self.fetchStationInfo(stationId: locationId, databaseManager: databaseManager) {
                ...
                // 获取星系和星域名称（多语言）
                if let systemInfo = await fetchSystemInfo(solarSystemId: stationInfo.system_id, databaseManager: databaseManager) {
                    securityStatus = systemInfo.security
                }
                ...
                if let systemInfo = await fetchSystemInfo(solarSystemId: structureInfo.solar_system_id, databaseManager: databaseManager) {
                    securityStatus = systemInfo.security
                    regionId = systemInfo.regionId
                }
            }
    ...
    private func createInitialRootNodes(
        ...
        while currentIndex < locationArray.count {
            // 创建任务组进行并发请求
            try await withThrowingTaskGroup(of: (Int64, String?, String?, Int?, Double?, Int?, Int?).self) { group in
                // 添加并发任务
                ...
                for locationId in locationArray[currentIndex..<endIndex] {
                    if let items = locationMap[locationId] {
                        ...
                        group.addTask {
                            ...
                            if locationType == "solar_system" {
                                // 如果是星系，使用星系图标
                                iconName = self.getSystemIcon(solarSystemId: Int(locationId), databaseManager: databaseManager)
                            } else {
                                // 其他类型使用建筑物图标
                                iconName = info.typeId != nil ? self.getStationIcon(typeId: info.typeId!, databaseManager: databaseManager) : nil
                            }
                ...
                // 收集结果
                for try await (locationId, locationName, iconName, typeId, securityStatus, systemId, regionId) in group {
                    if let items = locationMap[locationId] {
                        let locationType = items.first?.location_type ?? "unknown"
