...
public class CharacterStandingsAPI {
    ...
    public func fetchStandings(characterId: Int, forceRefresh: Bool = false) async throws
        -> [Standing]
    {
        // 如果不是强制刷新，先检查缓存
        if !forceRefresh {
        ...
        let urlString =
            "https://esi.evetech.net/characters/\(characterId)/standings/?datasource=tranquility"
    ...
    private func loadFromCache(characterId: Int) throws -> [Standing]? {
        ...
        Logger.info(
            "使用缓存的声望数据 - 角色ID: \(characterId), 记录数: \(standings.count), 缓存时间: \(Int(timeSinceModification))秒"
        )
        return standings
    }
