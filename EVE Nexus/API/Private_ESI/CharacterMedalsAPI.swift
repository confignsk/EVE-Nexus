...
public class CharacterMedalsAPI {
    ...
    public func fetchCharacterMedals(characterId: Int) async throws -> [CharacterMedal]? {
        ...
        let urlString = "https://esi.evetech.net/latest/characters/\(characterId)/medals/?datasource=tranquility"
    ...
    private func getCachedMedals(characterId: Int) -> [CharacterMedal]? {
        ...
        guard let data = UserDefaults.standard.data(forKey: key),
              ...
              Date().timeIntervalSince(cache.timestamp) < cacheTimeout else {
            return nil
        }
        return cache.medals
    }
