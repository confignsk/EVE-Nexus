...
class GetCorpContacts {
    ...
    // 获取单页联系人数据
    private func fetchContactsPage(characterId: Int, corporationId: Int, page: Int) async throws
        -> [ContactInfo]
    {
        let url = URL(
            string:
                "https://esi.evetech.net/latest/corporations/\(corporationId)/contacts/?datasource=tranquility&page=\(page)"
        )!
        ...
        while shouldContinue {
            do {
                let pageContacts = try await fetchContactsPage(
                    characterId: characterId, corporationId: corporationId, page: currentPage
                )
