...
public class CorporationAssetsJsonAPI {
    ...
    public func generateAssetTreeJson(
        ...
        // 1. 检查缓存
        if !forceRefresh {
            if let cacheFile = getCacheFilePath(corporationId: corporationId),
        ...
        if let jsonString = try await buildAssetTreeJson(
            ...
            // 保存到缓存
            saveToCache(jsonString: jsonString, corporationId: corporationId)
            // 通知进度完成
    ...
    private func saveToCache(jsonString: String, corporationId: Int) {
    ...
    private func buildAssetTreeJson(
        ...
        do {
            ...
            Logger.error("生成军团资产树JSON失败: \(error)")
            return nil
        }
    }
            ...
            try await withThrowingTaskGroup(
                ...
                for try await (
                    ...
                    if let items = locationMap[locationId] {
                        ...
                        default:
                            // 其他是玩家自定义名称的位置，保留名称
                            // 如果无法获取结构体名称但有typeId，从数据库查询物品名称作为回退
