    ...
    enum ResourceType: String, CaseIterable {
        ...
        var displayName: String {
            ...
            case .factionIcons:
                ...
                var name = NSLocalizedString(
                    "Main_Setting_Static_Resource_Faction_Icons", comment: ""
                )
                if stats.exists {
                    ...
                    if count > 0 {
                        name += String(
                            format: NSLocalizedString(
                                "Main_Setting_Static_Resource_Icon_Count", comment: ""
                            ), count
                        )
                    }
                }
            ...
            case .netRenders:
                ...
                var name = NSLocalizedString(
                    "Main_Setting_Static_Resource_Net_Renders", comment: ""
                )
                if stats.exists {
            ...
            case .characterPortraits:
                ...
                var name = NSLocalizedString(
                    "Main_Setting_Static_Resource_Character_Portraits", comment: ""
                )
                if stats.exists {
            ...
            if let enumerator = fileManager.enumerator(atPath: renderPath.path) {
                for case let fileName as String in enumerator {
                    if fileName.hasSuffix(".png") {
                        renderCount += 1
                        let filePath = (renderPath.path as NSString).appendingPathComponent(
                            fileName)
                        do {
        ...
        if exists {
            if let enumerator = fileManager.enumerator(atPath: portraitsPath.path) {
                for case let fileName as String in enumerator {
                    if fileName.hasSuffix(".png") {
                        portraitCount += 1
                        let filePath = (portraitsPath.path as NSString).appendingPathComponent(
                            fileName)
                        do {
                            let attributes = try fileManager.attributesOfItem(atPath: filePath)
                            totalSize += attributes[.size] as? Int64 ?? 0
            ...
            for case let fileURL as URL in enumerator {
                if fileURL.pathExtension == "png" {
                    do {
                        ...
                        if let modificationDate = attributes[FileAttributeKey.modificationDate]
                            as? Date
                        {
