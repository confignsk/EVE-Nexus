...
struct LogViewer: View {
    ...
    var body: some View {
        ...
        .alert(
            NSLocalizedString("Main_Setting_Logs_Delete_Title", comment: ""),
...
struct LogContentView: View {
        ...
        VStack(spacing: 0) {
            ...
            HStack {
                Image(systemName: "magnifyingglass")
                    .foregroundColor(.gray)
                TextField(
                    NSLocalizedString("Main_Setting_Logs_Search_Placeholder", comment: ""),
                    text: $searchText
                )
                    ...
                    LazyVStack(alignment: .leading, spacing: 2) {
                        ForEach(Array(filteredLines.enumerated()), id: \.offset) { index, line in
                            Text(line)
                                ...
                                .foregroundColor(
                                    isLineHighlighted(index: index) ? .white : .primary
                                )
                                .background(
                                    isLineHighlighted(index: index) ? Color.blue : Color.clear
                                )
                                .onAppear {
    ...
    private func isLineHighlighted(index: Int) -> Bool {
        guard let selectedIndex = selectedSearchResult,
            searchResults[safe: selectedIndex] != nil
        else {
            return false
        }
        ...
        while allLines.count < targetLines {
            ...
            if !data.isEmpty && !data.last!.isNewline {
                if let lastLine = lines.last, let lastLineData = lastLine.data(using: .utf8) {
                    try fileHandle.seek(
                        toOffset: fileHandle.offsetInFile - UInt64(lastLineData.count))
                    lines.removeLast()
                }
            }
