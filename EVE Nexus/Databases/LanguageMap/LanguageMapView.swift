...
struct LanguageMapView: View {
        ...
        VStack {
            ...
            HStack(spacing: 12) {
                HStack {
                    ...
                    TextField(NSLocalizedString("Main_Language_Map_Search", comment: ""), text: $searchText)
                        .submitLabel(.search)
            ...
            List {
                ...
                if !exactMatchResults.isEmpty {
                    Section(
                        header: Text(NSLocalizedString("Main_Language_Map_Exact_Match", comment: "完全匹配"))
                            .fontWeight(.bold)
                            .font(.system(size: 18))
                ...
                if !prefixMatchResults.isEmpty {
                    Section(
                        header: Text(NSLocalizedString("Main_Language_Map_Prefix_Match", comment: "前缀匹配"))
                            .fontWeight(.bold)
                            .font(.system(size: 18))
                            .foregroundColor(.primary)
                            .textCase(nil)
                ...
                if !fuzzyMatchResults.isEmpty {
                    Section(
                        header: Text(NSLocalizedString("Main_Language_Map_Fuzzy_Match", comment: "模糊匹配"))
                            .fontWeight(.bold)
                            .font(.system(size: 18))
                            .foregroundColor(.primary)
                            .textCase(nil)
    ...
    private func createQueryParameters(searchText: String, startPattern: String, searchPattern: String) -> [Any] {
        // 第一部分：完全匹配参数 (优先级 1)
        ...
        // 按照 SQL 查询中的参数顺序组合
        let allParams = exactMatchParams + // WHERE 子句的完全匹配
    ...
    private func performSearch() {
        ...
        // 创建查询参数
        let queryParams = createQueryParameters(searchText: searchText, startPattern: startPattern, searchPattern: searchPattern)
        ...
        """
        if case .success(let rows) = DatabaseManager.shared.executeQuery(typesQuery, parameters: queryParams) {
            for row in rows {
        ...
        """
        if case .success(let rows) = DatabaseManager.shared.executeQuery(systemsQuery, parameters: queryParams) {
            for row in rows {
                var names: [String: String] = [:]
        ...
        """
        if case .success(let rows) = DatabaseManager.shared.executeQuery(constellationsQuery, parameters: queryParams) {
            for row in rows {
                var names: [String: String] = [:]
        ...
        """
        if case .success(let rows) = DatabaseManager.shared.executeQuery(regionsQuery, parameters: queryParams) {
            for row in rows {
                var names: [String: String] = [:]
        ...
        let factionsQuery = """
            ...
            ORDER BY priority, name_length, en_name
        """
        if case .success(let rows) = DatabaseManager.shared.executeQuery(factionsQuery, parameters: queryParams) {
            for row in rows {
                var names: [String: String] = [:]
                if let deName = row["de_name"] as? String { names["de"] = deName }
                if let enName = row["en_name"] as? String { names["en"] = enName }
        ...
        let npcCorpsQuery = """
            ...
            ORDER BY priority, name_length, en_name
        """
        if case .success(let rows) = DatabaseManager.shared.executeQuery(npcCorpsQuery, parameters: queryParams) {
            for row in rows {
                var names: [String: String] = [:]
                if let deName = row["de_name"] as? String { names["de"] = deName }
                if let enName = row["en_name"] as? String { names["en"] = enName }
