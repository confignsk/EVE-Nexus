...
struct MarketQuickbarDetailView: View {
    ...
    var body: some View {
        ...
        .alert(
            ...
            Text(String(format: NSLocalizedString("Main_Market_Clipboard_Import_Confirm_Message", comment: ""), quickbar.items.count))
        }
        ...
        MarketItemSelectorBaseView(
            ...
            searchQuery: { text in
                ...
                if let typeIDs = allowTypeIDs, !typeIDs.isEmpty {
                    // 如果有物品ID白名单，添加物品ID筛选条件
                    let typeIDsString = typeIDs.map { String($0) }.joined(separator: ",")
                    return "t.marketGroupID \(groupIDsStringIn) AND t.type_id IN (\(typeIDsString)) AND (t.name LIKE ? OR t.en_name LIKE ? OR t.type_id = ?)"
                } else {
                    // 否则只筛选市场分组
                    return "t.marketGroupID \(groupIDsStringIn) AND (t.name LIKE ? OR t.en_name LIKE ? OR t.type_id = ?)"
                }
            },
            searchParameters: { text in
                ["%\(text)%", "%\(text)%", text]
...
class MarketClipboardParser {
    ...
    static func parseClipboardContent(
        ...
        let format2Regex = try! NSRegularExpression(pattern: #"^(.+?)\s+([\d,]+)(\s+|$)"#, options: [])
            ...
            if let match = format1Regex.firstMatch(in: trimmedLine, options: [], range: range) {
                if let quantityRange = Range(match.range(at: 1), in: trimmedLine),
                   let nameRange = Range(match.range(at: 2), in: trimmedLine) {
                    ...
                    itemName = String(trimmedLine[nameRange]).trimmingCharacters(in: .whitespacesAndNewlines)
                }
            }
        ...
        return ParseResult(successCount: successCount, failedItems: failedItems, updatedItems: updatedItems)
    }
    
    // 根据物品名称获取type_id
    private static func getTypeIDsForNames(_ names: [String], databaseManager: DatabaseManager) -> [String: Int] {
