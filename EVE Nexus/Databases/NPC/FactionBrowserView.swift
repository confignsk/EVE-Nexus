...
struct FactionBrowserView: View {
                                ...
                                NavigationLink(
                                    ...
                                    HStack {
                                        ...
                                        VStack(alignment: .leading, spacing: 2) {
                                            ...
                                            // 显示所属势力
                                            if let factionName = factions.first(where: { $0.id == corporation.factionId })?.name {
                                                Text(factionName)
                                                    .font(.caption)
                                                    .foregroundColor(.secondary)
                                            }
    ...
    private func loadAllFactions() async -> [FactionItem] {
        let query = "SELECT id, name, en_name, zh_name, shortDescription, description, iconName FROM factions"
    ...
    private func loadAllCorporations() async -> [CorporationItem] {
        let query = "SELECT corporation_id, name, en_name, zh_name, description, icon_filename, faction_id FROM npcCorporations"
            ...
            for row in rows {
                if let corporationId = row["corporation_id"] as? Int,
                   ...
                   let factionId = row["faction_id"] as? Int {
                    corporations.append(CorporationItem(
                        id: corporationId,
                        name: name,
                        enName: enName,
                        zhName: zhName,
                        description: description,
                        iconFileName: iconFileName.isEmpty ? "corporation_default" : iconFileName,
                        factionId: factionId
                    ))
                }
            }
    ...
    private func loadStandings() async {
        ...
        do {
            let standingsData = try await CharacterStandingsAPI.shared.fetchStandings(characterId: characterId)
