...
struct FacilityRigSelectorView: View {
    ...
    init(
        ...
        let equipmentData = loadEquipmentData(databaseManager: databaseManager, facilityTypeID: facilityTypeID)
        ...
        let allowedGroupIDs = MarketManager.shared.getAllSubGroupIDsFromID(marketGroups, startingFrom: 2203)
        self._allowedMarketGroupIDs = State(initialValue: Set(allowedGroupIDs))
    }
        ...
        NavigationStack {
            if self.allowedTypeIDs.isEmpty {
                ContentUnavailableView {
                    Label(
                        NSLocalizedString("Facility_Rig_Selector_No_Compatible_Rigs", comment: "该建筑不支持插件"),
                ...
                MarketItemTreeSelectorView(
                    ...
                    onItemSelected: { item in
                        // 调用回调函数选择插件
                        Logger.info("用户选择了插件: \(item.name), ID: \(item.id), 建筑ID: \(facilityTypeID)")
    ...
    private func loadEquipmentData(databaseManager: DatabaseManager, facilityTypeID: Int) -> [EquipmentInfo] {
