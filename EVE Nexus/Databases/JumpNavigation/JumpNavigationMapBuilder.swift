...
class JumpNavigationHandler {
    ...
    // 计算两点之间的距离（光年）
    static func calculateDistanceLY(x1: Double, y1: Double, z1: Double, x2: Double, y2: Double, z2: Double) -> Double {
        // 计算欧几里得距离
    ...
    // 获取所有符合条件的星系对
    static func getNearbySystems(databaseManager: DatabaseManager, progressUpdate: ((String, Double) -> Void)? = nil) -> [[String: Any]] {
        ...
        progressUpdate?(NSLocalizedString("Jump_Navigation_Processing_System_Data", comment: ""), 0.01)
        ...
        // 通知进度：开始计算距离 - 计算过程占进度98%
        progressUpdate?(NSLocalizedString("Jump_Navigation_Calculating_Jump_Distance", comment: ""), 0.01)
        ...
        for i in 0..<filteredSystems.count {
            for j in (i+1)..<filteredSystems.count {
                ...
                if processedPairs % 500 == 0 {
                    progressUpdate?(NSLocalizedString("Jump_Navigation_Calculating_Jump_Distance", comment: ""), progress)
                }
    ...
    // 处理跳跃导航数据
    static func processJumpNavigationData(databaseManager: DatabaseManager, progressUpdate: ((String, Double) -> Void)? = nil) {
        // 获取结果
        let results = getNearbySystems(databaseManager: databaseManager, progressUpdate: progressUpdate)
    ...
    // 获取所有合法星系信息（用于选择器）
    static func getAllJumpableSystems(databaseManager: DatabaseManager) -> [(id: Int, name: String, security: Double)] {
        // 使用全局缓存数据
        JumpSystemsCache.shared.loadIfNeeded(databaseManager: databaseManager)
