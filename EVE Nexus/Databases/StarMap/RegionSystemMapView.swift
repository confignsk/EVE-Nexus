...
struct RegionSystemMapView: View {
            ...
            if !isLoading && !systemNodes.isEmpty {
                HStack {
                    HStack {
                        ...
                        TextField(NSLocalizedString("StarMap_Search_System", comment: "搜索星系"), text: $searchText)
    ...
    private func loadMapData() -> SystemMapData? {
        guard let url = Bundle.main.url(forResource: "systems_data", withExtension: "json"),
              ...
              let mapData = try? JSONDecoder().decode(SystemMapData.self, from: jsonData) else {
    ...
    private func querySystemInfo(systemIds: [Int]) -> [Int: (name: String, security: Double, regionId: Int)] {
        ...
        guard case let .success(rows) = databaseManager.executeQuery(sql, parameters: systemIds.map { $0 as Any }) else {
    ...
    private func buildSystemNodes(mapData: SystemMapData, systemInfo: [Int: (name: String, security: Double, regionId: Int)]) -> [SystemNodeData] {
        ...
        for (systemIdStr, position) in mapData.systems {
            guard let systemId = Int(systemIdStr),
                  let info = systemInfo[systemId] else { continue }
        ...
        for system in systemNodes {
            ...
            for connectionId in system.connections {
                if let targetSystem = systemNodes.first(where: { $0.systemId == connectionId }) {
                    ...
                    if isCrossRegion {
                        // 跨星域连线使用虚线
                        context.stroke(linePath, with: gradientShading, style: StrokeStyle(
    ...
    private struct SystemNodeView: View {
        ...
        private var isFiltered: Bool {
            !parentView.searchText.isEmpty && parentView.filteredNodes.contains { $0.systemId == systemId }
        }
    ...
    private func centerOnSystem(_ system: SystemNodeData) {
        let bounds = calculateBounds()
        let centerX = UIScreen.main.bounds.width / 2 / (transform.scale * 2.0) - (bounds.minX + bounds.width / 2)
        let centerY = UIScreen.main.bounds.height / 2 / (transform.scale * 2.0) - (bounds.minY + bounds.height / 2)
