...
struct RegionMapView: View {
                    ...
                    if isLoading {
                        VStack {
                            ProgressView(NSLocalizedString("StarMap_Loading", comment: "Loading star map"))
                                .padding()
                            Text(NSLocalizedString("StarMap_Processing", comment: "Processing star map data"))
                                .font(.caption)
                        ...
                        Canvas { context, size in
                            ...
                            ForEach(regionData, id: \.region_id) { region in
                                RegionNodeView(
                                    regionName: regionNames[region.region_id] ?? NSLocalizedString("StarMap_Unknown_Region", comment: "Unknown"),
        ...
        .navigationDestination(isPresented: $showRegionDetail) {
            if let selectedRegionId = selectedRegionId,
               let regionName = regionNames[selectedRegionId] {
                RegionSystemMapView(databaseManager: databaseManager, regionId: selectedRegionId, regionName: regionName)
            }
        }
        .alert(NSLocalizedString("StarMap_Export_Success", comment: "Export Success"), isPresented: $showExportSuccess) {
            Button("OK") { }
        } message: {
            Text(NSLocalizedString("StarMap_Export_Success_Message", comment: "Map exported to photo library"))
        }
        .alert(NSLocalizedString("StarMap_Export_Error", comment: "Export Failed"), isPresented: $showExportError) {
            Button("OK") { }
        } message: {
        ...
        SimultaneousGesture(
            ...
            DragGesture(minimumDistance: 15) // 设置最小拖拽距离，避免与点击冲突
                .onChanged { value in
                    ...
                    transform.offset = constrainOffset(newOffset, scale: transform.scale, viewSize: viewSize)
                }
                .onEnded { _ in
    ...
    private func handleNodeTap(at location: CGPoint, viewSize: CGSize) {
        ...
        let centerX = viewSize.width / 2 / (transform.scale * 1.0) - (bounds.minX + bounds.width / 2)
        let centerY = viewSize.height / 2 / (transform.scale * 1.0) - (bounds.minY + bounds.height / 2)
        ...
        for region in regionData {
            ...
            if distanceX <= halfWidth && distanceY <= halfHeight {
                ...
                let distance = sqrt(pow(mapLocation.x - regionPoint.x, 2) + pow(mapLocation.y - regionPoint.y, 2))
    ...
    // MARK: - 拖动范围限制 (基于内容边界)
    private func constrainOffset(_ offset: CGSize, scale: CGFloat, viewSize: CGSize) -> CGSize {
        ...
        // 计算最大可偏移量（确保节点完全可见）
        let maxOffsetX = max(0, (scaledContentWidth - viewSize.width) / 2 + scaledNodeWidth / 2 + 20)
        let maxOffsetY = max(0, (scaledContentHeight - viewSize.height) / 2 + scaledNodeHeight / 2 + 20)
    ...
    private func loadData() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            let factionNames = self.queryFactionNames(factionIds: Array(Set(regions.compactMap { $0.faction_id != 0 ? $0.faction_id : nil })))
    ...
    private func loadRegionData() -> [RegionData] {
        guard let url = Bundle.main.url(forResource: "regions_data", withExtension: "json"),
              ...
              let regions = try? JSONDecoder().decode([RegionData].self, from: data) else {
    ...
    private func queryRegionNames(regionIds: [Int]) -> [Int: String] {
        let sql = "SELECT regionID, regionName FROM regions WHERE regionID IN (\(regionIds.map(String.init).joined(separator: ",")))"
    ...
    private func queryFactionNames(factionIds: [Int]) -> [Int: String] {
        ...
        let sql = "SELECT id, name FROM factions WHERE id IN (\(factionIds.map(String.init).joined(separator: ",")))"
    ...
    private func drawStarMapNative(context: GraphicsContext, size: CGSize) {
        ...
        context.drawLayer { layerContext in
            ...
            // 计算居中偏移
            let bounds = calculateBounds()
            let centerX = size.width / 2 / (transform.scale * 1.0) - (bounds.minX + bounds.width / 2)
            let centerY = size.height / 2 / (transform.scale * 1.0) - (bounds.minY + bounds.height / 2)
    ...
    private func calculateBounds() -> (minX: Double, maxX: Double, minY: Double, maxY: Double, width: Double, height: Double) {
    ...
    private func drawConnectionsNative(context: GraphicsContext) {
        for region in regionData {
            ...
            let fromColor = getRegionBorderColor(regionId: region.region_id, factionId: region.faction_id)
            ...
            for relationId in region.relations {
                if let targetRegion = regionData.first(where: { String($0.region_id) == relationId }) {
                    ...
                    let toColor = getRegionBorderColor(regionId: targetRegion.region_id, factionId: targetRegion.faction_id)
                    ...
                    context.stroke(linePath, with: gradientShading, lineWidth: 1.2 / transform.scale)
                }
            }
        }
    }
        ...
        for region in regionData {
            ...
            if let nodeSymbol = context.resolveSymbol(id: region.region_id) {
                context.drawLayer { nodeContext in
                    ...
                    nodeContext.draw(nodeSymbol, at: CGPoint(
    ...
    private func zoomIn() {
        withAnimation(.easeOut(duration: 0.2)) {
            transform.scale = min(transform.scale * 1.3, 5.0)
    ...
    private func zoomOut() {
        withAnimation(.easeOut(duration: 0.2)) {
            transform.scale = max(transform.scale / 1.3, 0.2)
            transform.lastScale = transform.scale
        }
    }
