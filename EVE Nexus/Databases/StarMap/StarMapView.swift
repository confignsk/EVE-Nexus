...
struct StarMapView: View {
    ...
    var body: some View {
            ...
            List {
                ...
                Section {
                    NavigationLink(value: "region_map") {
                        Label(
                            NSLocalizedString("Main_Star_Map", comment: "星域图"), systemImage: "map"
                        )
                    }
                }
                // Section 2: 星域列表
                Section(
                    header: Text(NSLocalizedString("Main_Language_Map_Type_Region", comment: ""))
                ) {
                    if filteredRegions.isEmpty && !searchText.isEmpty {
                        ...
                        ForEach(filteredRegions) { region in
                            NavigationLink(
                                value: RegionNavigation.regionMap(region.id, region.name)
                            ) {
                                Text(region.name)
                            }
                        }
        ...
        .navigationDestination(for: RegionNavigation.self) { navigation in
            switch navigation {
            case let .regionMap(regionId, regionName):
                RegionSystemMapView(
                    databaseManager: databaseManager, regionId: regionId, regionName: regionName
                )
            }
        }
    ...
    private func loadRegionData() {
        ...
        do {
            ...
            availableRegionIds = Set(regionDataArray.map { $0.region_id })
        ...
        if case let .success(rows) = DatabaseManager.shared.executeQuery(sql) {
            let allRegionsData = rows.compactMap { (row: [String: Any]) -> StarMapRegion? in
                guard let id = row["regionID"] as? Int,
                      ...
                      availableRegionIds.contains(id)
                else {
                    return nil
                }
