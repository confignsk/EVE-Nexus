...
struct HiSlotEquipmentSelectorView: View {
    ...
    init(
        ...
        // 使用飞船ID构建键名
        let hiSlotGroupIDKey =
            shipTypeID > 0
                ? "LastVisitedHighSlotGroupID_\(shipTypeID)" : "LastVisitedHighSlotGroupID"

        // 尝试从 UserDefaults 加载上次访问的组ID
        if let savedGroupID = UserDefaults.standard.object(forKey: hiSlotGroupIDKey) as? Int {
            Logger.info("从 UserDefaults 加载到之前保存的高槽装备目录ID: \(savedGroupID), 飞船ID: \(shipTypeID)")
            _lastVisitedGroupID = State(initialValue: savedGroupID)
        } else {
            Logger.info("未找到保存的高槽装备目录ID，飞船ID: \(shipTypeID)")
            _lastVisitedGroupID = State(initialValue: nil)
        }

        // 使用飞船ID构建搜索键名
        let hiSlotSearchKey =
            shipTypeID > 0 ? "LastHighSlotSearchKeyword_\(shipTypeID)" : "LastHighSlotSearchKeyword"

        // 尝试从 UserDefaults 加载上次搜索关键词
        if let savedKeyword = UserDefaults.standard.string(forKey: hiSlotSearchKey) {
            ...
            if self.allowedTypeIDs.isEmpty {
                ...
                MarketItemTreeSelectorView(
                    ...
                    onItemSelected: { item in
                        ...
                        let hiSlotGroupIDKey =
                            shipTypeID > 0
                                ? "LastVisitedHighSlotGroupID_\(shipTypeID)"
                                : "LastVisitedHighSlotGroupID"
                        let hiSlotSearchKey =
                            shipTypeID > 0
                                ? "LastHighSlotSearchKeyword_\(shipTypeID)"
                                : "LastHighSlotSearchKeyword"

                        // 保存当前组ID到UserDefaults
                        if let groupID = item.marketGroupID {
                            self.lastVisitedGroupID = groupID
                            Logger.info("保存高槽装备导航目录ID: \(groupID), 飞船ID: \(shipTypeID)")
                            UserDefaults.standard.set(groupID, forKey: hiSlotGroupIDKey)
                            // 选择装备时清空搜索关键词
                            UserDefaults.standard.removeObject(forKey: hiSlotSearchKey)
                            lastSearchKeyword = nil
                        }
                        // 仅在slotFlag有效时记录日志
                        Logger.info("用户选择了高槽装备，槽位标识：\(slotFlag.rawValue)")
                        // 直接调用回调函数安装装备，不再需要进行slotFlag的检查
                    ...
                    onDismiss: { _, searchText in
                        ...
                        if let searchText = searchText, !searchText.isEmpty {
                            Logger.info("保存高槽装备搜索关键词: \"\(searchText)\", 飞船ID: \(shipTypeID)")
                            UserDefaults.standard.set(searchText, forKey: hiSlotSearchKey)
                        ...
                        if !hasSelectedItem {
                            ...
                            UserDefaults.standard.removeObject(forKey: hiSlotGroupIDKey)
                ...
                .onAppear {
                    Logger.info("高槽装备选择器显示，飞船ID: \(shipTypeID)")
                    // 重置选择状态
    ...
    private func loadEquipmentData(databaseManager: DatabaseManager) -> [EquipmentInfo] {
        ...
        if case let .success(rows) = databaseManager.executeQuery(query) {
            ...
            Logger.info("加载了 \(equipmentInfos.count) 个高槽装备")
        } else {
            Logger.error("加载高槽装备信息失败")
        }
