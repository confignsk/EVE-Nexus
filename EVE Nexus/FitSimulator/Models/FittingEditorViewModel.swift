...
class FittingEditorViewModel: ObservableObject {
    ...
    // 无人机属性计算结果
    var droneAttributes: (bandwidth: (current: Double, total: Double), capacity: (current: Double, total: Double), dronesCount: Int, activeDronesCount: Int) {
        // 计算无人机带宽
        ...
        for drone in simulationInput.drones {
            if drone.activeCount > 0 {
                ...
                if let simulationOutput = simulationOutput,
                   let droneIndex = simulationOutput.drones.firstIndex(where: { $0.typeId == drone.typeId }),
                   let bandwidth = simulationOutput.drones[droneIndex].attributesByName["droneBandwidthUsed"] {
    ...
    init(shipTypeId: Int, shipInfo: (name: String, iconFileName: String), databaseManager: DatabaseManager) {
        ...
        // 转换为模拟输入
        self.simulationInput = FitConvert.localFittingToSimulationInput(
    ...
    init(fittingId: Int, databaseManager: DatabaseManager) {
        ...
        do {
            ...
            self.simulationInput = SimulationInput(
                ...
                description: "",
                fighters: nil,
                ship: SimShip(typeId: 0, baseAttributes: [:], baseAttributesByName: [:], effects: [], groupID: 0, name: "Unknown", iconFileName: "items_7_64_15.png", requiredSkills: []),
                modules: [],
                drones: [],
    ...
    init(onlineFitting: CharacterFitting, databaseManager: DatabaseManager) {
        ...
        // 查询飞船信息
        let shipQuery = "SELECT name, icon_filename FROM types WHERE type_id = ?"
        if case let .success(rows) = databaseManager.executeQuery(shipQuery, parameters: [onlineFitting.ship_type_id]), 
           let row = rows.first,
           let name = row["name"] as? String,
           let iconFileName = row["icon_filename"] as? String {
            self.shipInfo = (name: name, iconFileName: iconFileName)
        ...
        do {
            ...
            Logger.info("将在线配置转换为JSON: 飞船ID=\(onlineFitting.ship_type_id), 配置项数量=\(fittingItems.count)")
            if let localFittings = try? FitConvert.online2local(jsonData: jsonData),
               let localFitting = localFittings.first {
                ...
                self.simulationInput = finalSimInput
                self.invalidModules = invalidModules
                ...
                throw NSError(domain: "FittingEditorViewModel", code: 1, userInfo: [NSLocalizedDescriptionKey: "在线配置转换失败"])
            }
        } catch {
            // 如果转换失败，使用默认值
    ...
    func calculateDynamicHardpoints() -> (turretHardpoints: Int, launcherHardpoints: Int) {
        // 获取基础挂点数
            let baseTurretHardpoints = Int(simulationInput.ship.baseAttributesByName["turretSlotsLeft"] ?? 0)
            let baseLauncherHardpoints = Int(simulationInput.ship.baseAttributesByName["launcherSlotsLeft"] ?? 0)
        ...
        Logger.info("动态挂点计算结果 - 炮台挂点: \(totalTurretHardpoints) (基础: \(baseTurretHardpoints)), 发射器挂点: \(totalLauncherHardpoints) (基础: \(baseLauncherHardpoints))")
    ...
    func calculateAttributes() {
        ...
        if let fighters = simulationInput.fighters {
            ...
            for fighter in fighters {
                Logger.info("【calculateAttributes】输入舰载机: \(fighter.name), typeId: \(fighter.typeId), 属性数量: \(fighter.attributesByName.count)")
            }
        } else {
        ...
        if let outputFighters = simulationOutput?.fighters {
            ...
            for fighter in outputFighters {
                Logger.info("【calculateAttributes】输出舰载机: \(fighter.name), typeId: \(fighter.typeId), 属性数量: \(fighter.attributesByName.count)")
                
                // 检查伤害属性
                let damageAttributes = fighter.attributesByName.filter { $0.key.lowercased().contains("damage") }
        ...
        for flag in flags {
            if let index = simulationInput.modules.firstIndex(where: { $0.flag == flag }) {
                ...
                if finalStatus != currentModule.status {
                    ...
                    Logger.info("批量更新模块状态: \(currentModule.name), 从 \(currentModule.status) 到 \(finalStatus) (目标: \(newStatus), 最大: \(maxStatus))")
                } else {
        ...
        if !updatedModules.isEmpty {
            ...
            for (groupID, modules) in groupedModules {
                if let firstModule = modules.first {
                    ...
                    if let simulationOutput = simulationOutput,
                       let outputModule = simulationOutput.modules.first(where: { $0.groupID == groupID }) {
                        // 从计算后的输出中获取
                        maxGroupOnline = Int(outputModule.attributesByName["maxGroupOnline"] ?? 0)
                        maxGroupActive = Int(outputModule.attributesByName["maxGroupActive"] ?? 0)
                    } else {
    ...
    func batchInstallCharge(typeId: Int, name: String, iconFileName: String?, flags: [FittingFlag]) {
        ...
        if case let .success(rows) = databaseManager.executeQuery(groupQuery, parameters: [typeId]) {
            if let row = rows.first, let id = row["groupID"] as? Int, let typeVolume = row["volume"] as? Double, typeVolume > 0 {
                groupId = id
                volume = typeVolume
            }
        }
    ...
    func installModule(typeId: Int, flag: FittingFlag, status: Int = 0) {
        ...
        // 查询装备分组和体积
        let groupQuery = "SELECT name, icon_filename, groupID, volume, capacity FROM types WHERE type_id = ?"
    ...
    /// 添加无人机到配置中
    func addDrone(typeId: Int, name: String, iconFileName: String?, quantity: Int, activeCount: Int = 0) {
        // 获取无人机信息
    ...
    func activateDrone(typeId: Int) {
        ...
        // 检查是否达到最大激活数量
        let currentActive = droneAttributes.activeDronesCount
        if currentActive >= maxActiveDrones {
            return
        }
        
        // 检查带宽是否足够
        let droneBandwidth = drone.attributesByName["droneBandwidthUsed"] ?? 0
        let remainingBandwidth = droneAttributes.bandwidth.total - droneAttributes.bandwidth.current
        
        // 如果带宽不足，无法激活
        if droneBandwidth > remainingBandwidth {
            return
        }
        ...
        let updatedDrone = SimDrone(
            ...
            activeCount: drone.activeCount + 1,
        ...
        Logger.info("激活无人机成功: \(drone.name), 当前激活数量: \(updatedDrone.activeCount)")
    }
    
    /// 停用无人机
    func deactivateDrone(typeId: Int) {
        // 检查是否存在该类型的无人机
        guard let index = simulationInput.drones.firstIndex(where: { $0.typeId == typeId }) else {
            return
        }
        ...
        Logger.info("停用无人机成功: \(drone.name), 当前激活数量: \(updatedDrone.activeCount)")
    }
    
    /// 获取无人机信息
    func getDroneInfo(typeId: Int) -> (attributes: [Int: Double], attributesByName: [String: Double], effects: [Int], volume: Double, bandwidth: Double, groupID: Int)? {
        var attributes: [Int: Double] = [:]
        var attributesByName: [String: Double] = [:]
        var effects: [Int] = []
        var volume: Double = 0
        ...
        return (attributes: attributes, attributesByName: attributesByName, effects: effects, volume: volume, bandwidth: bandwidth, groupID: groupID)
    }
    ...
    func replaceDrone(oldTypeId: Int, newTypeId: Int) {
        // 检查是否存在旧无人机
        guard let index = simulationInput.drones.firstIndex(where: { $0.typeId == oldTypeId }) else {
            return
        }
        ...
        if let existingIndex = simulationInput.drones.firstIndex(where: { $0.typeId == newTypeId }) {
            ...
            if case let .success(rows) = databaseManager.executeQuery(query, parameters: [newTypeId]), 
               let row = rows.first {
                if let name = row["name"] as? String {
                    newName = name
                }
            ...
            let newDrone = SimDrone(
                ...
                requiredSkills: FitConvert.extractRequiredSkills(attributes: newDroneInfo.attributes),
                name: newName,
    ...
    func updateCargoItemQuantity(typeId: Int, quantity: Int) {
        // 检查是否存在该类型的物品
        guard let index = simulationInput.cargo.items.firstIndex(where: { $0.typeId == typeId }) else {
            return
        }
    ...
    func canLoadCharge(moduleTypeId: Int, chargeTypeId: Int) -> Bool {
        ...
        // 执行查询获取模块属性
        if case let .success(rows) = databaseManager.executeQuery(attrQuery, parameters: [moduleTypeId]) {
            for row in rows {
                if let name = row["name"] as? String,
                   let value = row["value"] as? Double {
                    // 收集chargeGroup属性
        ...
        // 获取弹药的组ID和大小
        var chargeGroupID: Int = 0
        var chargeSizeValue: Double? = nil
        
        // 查询弹药组ID
        let chargeGroupQuery = "SELECT groupID FROM types WHERE type_id = ?"
        if case let .success(rows) = databaseManager.executeQuery(chargeGroupQuery, parameters: [chargeTypeId]),
           let row = rows.first,
        ...
        if case let .success(rows) = databaseManager.executeQuery(chargeSizeQuery, parameters: [chargeTypeId]),
           let row = rows.first,
           let value = row["value"] as? Double {
    ...
    func getDatabaseItemInfo(typeId: Int) -> DatabaseListItem? {
        ...
        let results = databaseManager.loadMarketItems(whereClause: whereClause, parameters: [typeId], limit: 1)
    ...
    // 舰载机属性计算结果
    var fighterAttributes: (light: (used: Int, total: Int), heavy: (used: Int, total: Int), support: (used: Int, total: Int)) {
        // 获取各类型舰载机槽位数量
        ...
        let supportTotal = Int(simulationInput.ship.baseAttributesByName["fighterSupportSlots"] ?? 0)
    ...
    func updateFighterQuantity(tubeId: Int, quantity: Int) {
        guard let fighters = simulationInput.fighters,
              let index = fighters.firstIndex(where: { $0.tubeId == tubeId }) else { return }
