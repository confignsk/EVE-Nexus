...
class Step3 {
    ...
    /// 展示单个物品的修饰器信息
    private func displayItemModifiers(
        modifiers: [Int: [SimAttributeModifier]], indent: String = "  "
    ) {
        ...
        for attributeId in sortedAttributeIds {
            guard let attributeModifiers = modifiers[attributeId], !attributeModifiers.isEmpty
            else {
                continue
            }
            ...
            for (index, modifier) in attributeModifiers.enumerated() {
                Logger.info(
                    "\(indent)  [\(index + 1)] 来源: \(modifier.sourceName)(\(modifier.sourceTypeID ?? -1) | 操作: \(modifier.operation) | 叠加惩罚: \(modifier.stackingPenalty ? "是" : "否") | 修饰值:  \(modifier.value) | 操作: \(modifier.operation) | 来源属性ID: \(modifier.sourceAttributeId) | 来源效果ID: \(modifier.effectId ?? -1)"
                )
                totalModifiers += 1
            }
        }
