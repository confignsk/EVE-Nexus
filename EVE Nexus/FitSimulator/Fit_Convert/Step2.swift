...
class Step2 {
    ...
    func process(
        ...
        itemAttributesByName _: [Int: [String: Double]],
    ...
    private func parseModifierInfo(
        effect: EffectDetail,
        typeId: Int,
        itemAttributes _: [Int: [Int: Double]]
    ) -> [AttributeModifier] {
        ...
        guard let modifierData = modifierInfo.data(using: .utf8),
              let parsedModifiers = try? JSONDecoder().decode([ModifierInfo].self, from: modifierData)
        else {
        ...
        for modifier in parsedModifiers {
            ...
            // 解析修饰器类型
            guard
                let modifierType = parseModifierType(
                    func: modifier.func,
...
enum ModifierDomain: String {
    case shipID // 作用于所在的飞船
    case itemID // 作用于物品自身
    case charID // 作用于角色属性
    case otherID // 作用于关联物品
    case structureID // 作用于结构
    case targetID // 作用于目标
    case target // 作用于目标
}
